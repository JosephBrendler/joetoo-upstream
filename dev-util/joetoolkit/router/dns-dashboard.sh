...
while [ true ] ; do
  clear;
  j_msg -m "${BYon}#$(repeat '-' 79)${Boff}"
  j_msg -p  "${LBon}# ipv4 subset of /etc/hosts.d/20_openVPN_clients as of $(timestamp)${Boff}"
  j_msg -m  "${BYon}#$(repeat '-' 79)${Boff}"
  cat /etc/hosts.d/20_openVPN_clients | grep "${ipv4_filter}";
  sleep 5;
  clear;
  j_msg -m  "${BYon}#$(repeat '-' 79)${Boff}"
  j_msg -p  "${LBon}# ipv6 subset of /etc/hosts.d/20_openVPN_clients as of $(timestamp)${Boff}"
  j_msg -m  "${BYon}#$(repeat '-' 79)${Boff}"
  cat /etc/hosts.d/20_openVPN_clients | grep "${ipv6_filter}";
  sleep 5;
  clear;
  j_msg -m  "${BYon}#$(repeat '-' 89)${Boff}"
  j_msg -p  "${LBon}## /etc/hosts.d/30_ddns_clients as of $(timestamp)${Boff}"
  j_msg -m  "${BYon}#$(repeat '-' 89)${Boff}"
  cat /etc/hosts.d/30_ddns_clients;
  sleep 5;
  clear;
  j_msg -m  "${BYon}#$(repeat '-' 66)${Boff}"
  j_msg -p  "${LBon}## /var/lib/misc/dnsmasq.leases as of $(timestamp)${Boff}"
  j_msg -m  "${BYon}#$(repeat '-' 66)${Boff}"
  j_msg -mp  "(DUID is also available in /var/lib/misc/dnsmasq.leases)"
  j_msg -mp  "Note: addresses with hostname = '*' area probably for the same hostname as the line above,"
  j_msg -mp  "  but for a different interface (e.g. gmki91 should show for both wlan0 and eth0)"
